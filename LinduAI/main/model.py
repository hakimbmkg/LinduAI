...
class Models():
    ...
    def trainmodels(path, summary_models = None):
        data_dir = Path(path)
    ...
    def testmodels(path):
        ...
        file_names = str(datetime.time)+'.png'
        plt.savefig(directory+'/input/tmp_files/'+file_names, bbox_inches='tight', dpi=300)
        plt.close()
        Models.predicted(directory+'/input/tmp_files/'+file_names)

    def predicted(path):
        class_names = ['Baik','Buruk','Sedang'] 
        model = tf.keras.models.load_model('input/models_noise/')
